...
public class EmojiLoadManager {
    ...
    public void loadEmojiOutlineFrom(String path) {
        L.info(TAG, "method->loadEmojiOutlineFrom,path: " + path);
    ...
    private static File getFile(String path) {
        ...
        if (!directory.exists()) {
            L.error(TAG, "method->loadEmojiOutlineFrom,file is not exist");
            return null;
        }
        if (!directory.isDirectory()) {
            L.error(TAG, "method->loadEmojiOutlineFrom,file is not directory");
            return null;
        }
        return directory;
    }
    ...
    private static <T> T getFileManifest(File directory, Class<T> fileClass) {
        if (directory == null || !directory.isDirectory() || !directory.exists()) {
            L.error(TAG, "method->getFileManifest,file is illegal");
            return null;
        }
        ...
        if (manifestFile != null) {
            ...
            L.info(TAG, "method->getFileManifest,manifestFileName: " + manifestFileName);
    ...
    private static <T> T parse(String manifestFileName, Class<T> emojiClass) {
        ...
        if (!TextUtils.isEmpty(manifestContent)) {
            try {
                ...
                L.error(TAG, "method->parse,fail to parse json,exception: " + e);
            }
        }
        return null;
    }
